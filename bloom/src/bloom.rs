...
impl FalsePositiveRate {
    ...
    fn bits_per_item_q40(self) -> u64 {
        let index = self
            ...
            .checked_sub(1)
            .map(usize::from)
            .expect("false positive rate must be in 1..=20");
        Self::BITS_PER_ITEM_Q40[index]
    }
}
...
impl<T: BloomHashIndex> Bloom<T> {
    ...
    fn div_round(numerator: u64, denominator: u64) -> u64 {
        ...
        let quotient = numerator.checked_div(denominator).unwrap();
        let remainder = numerator.checked_rem(denominator).unwrap();
        quotient
            .checked_add(u64::from(remainder >= denominator.div_ceil(2)))
            .expect("div_round overflow")
    }
    ...
    fn num_keys(num_bits: u64, num_items: u64) -> u64 {
        let Some(whole) = num_bits.checked_div(num_items) else {
            return 0;
        };
        let remainder = num_bits.checked_rem(num_items).unwrap();
        let whole_q40 = whole
