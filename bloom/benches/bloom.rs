...
fn bench_bits_set_hasher(c: &mut Criterion) {
    let bits: BitVec<u8> = BitVec::new_fill(false, 38_340_234_u64);
    let mut hasher = FnvHasher::default();

    c.bench_function("bits_set_hasher", |b| {
        b.iter(|| {
            let idx = hasher.finish() % bits.len();
            hasher.write_u64(idx);
        })
...
fn bench_sigs_bloom(c: &mut Criterion) {
...
fn bench_sigs_hashmap(c: &mut Criterion) {
    ...
    c.bench_function("sigs_hashmap", |b| {
        b.iter(|| {
            id = hash(id.as_ref());
            let mut sigbytes = Vec::from(id.as_ref());
            id = hash(id.as_ref());
...
fn bench_add_hash(c: &mut Criterion) {
...
fn bench_add_hash_atomic(c: &mut Criterion) {
    let mut rng = rand::thread_rng();
    let hash_values: Vec<_> = std::iter::repeat_with(Hash::new_unique)
        .take(1200)
        .collect();
    ...
    c.bench_function("add_hash_atomic", |b| {
        b.iter(|| {
