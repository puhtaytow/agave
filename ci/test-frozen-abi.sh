...
#   cmd="cargo +$rust_nightly test -p solana-vote --features frozen-abi --lib -- test_abi_digest test_api_digest --nocapture"
#   echo "--- $cmd"
#   $cmd
# # done
...
packages=$(cargo +"$rust_nightly" metadata --no-deps --format-version=1 | jq -r '.packages[] | select(.features | has("frozen-abi")) | .name')
for package in $packages; do
  cmd="cargo +$rust_nightly test -p $package --features frozen-abi --lib -- test_abi_digest test_api_digest"
  echo "--- $cmd"
  $cmd
done