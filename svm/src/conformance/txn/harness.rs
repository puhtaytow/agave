...
use {
    crate::conformance::{
        callback::ConformanceCallback, direct_mapping::direct_mapping_handle_cu_exhaustion,
    },
...
fn add_builtins_to_transaction_batch_processor<FG: ForkGraph>(
...
#[cfg(feature = "conformance")]
fn instruction_error_is_precompile(
    status: &solana_transaction_error::TransactionResult<()>,
    transaction: &SanitizedTransaction,
) -> bool {
    let Err(TransactionError::InstructionError(index, InstructionError::Custom(_))) = status else {
        return false;
    };
    transaction
        .message()
...
pub fn execute_txn_proto(input: ProtoTxnContext) -> ProtoTxnResult {
    ...
    let zero_custom_error = instruction_error_is_precompile(&effects.status, &context.transaction);
    let mut output: ProtoTxnResult = effects.into();
    if zero_custom_error {
        output.custom_error = 0;
    }
    output
}
...
mod tests {
    ...
    #[cfg(feature = "conformance")]
    fn transaction_with_program(program_id: Pubkey) -> SanitizedTransaction {
    ...
    #[cfg(feature = "conformance")]
    #[test]
    fn test_precompile_custom_error_is_detected() {
        let transaction = transaction_with_program(solana_sdk_ids::secp256k1_program::id());
        let status = Err(TransactionError::InstructionError(
            0,
            InstructionError::Custom(7),
        ));

        assert!(instruction_error_is_precompile(&status, &transaction));
    }

    #[cfg(feature = "conformance")]
    #[test]
    fn test_non_precompile_custom_error_is_not_detected() {
        let transaction = transaction_with_program(Pubkey::new_unique());
        let status = Err(TransactionError::InstructionError(
            0,
            InstructionError::Custom(7),
        ));
